class MatchesController < ApplicationController
  ...
  def index
    if params[:search].present?
      @matches = Match.where("first_team LIKE ? OR secod_team LIKE ?", "%#{params[:search]}%", "%#{params[:search]}%").page(params[:page])
    else
  ...
  private
    ...
    def match_params
      params.require(:match).permit(:first_team, :secod_team, :first_team_score, :second_team_score)
    end
end
